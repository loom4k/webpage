...
export const getPostData = async (no: number): Promise<IPostData> => {
	...
	const labels: ILabel[] = data.repository.discussion.labels.edges.map(
		(l: { node: { color: any; name: any; id: any; }; }) => {
			return {
				color: l.node.color,
				name: l.node.name,
				id: l.node.id,
...
export const getPinnedRepos = async (): Promise<IStarredRepo[]> => {
	...
	const repos: IStarredRepo[] = data.user.pinnedItems.nodes.map((r: { description: any; name: any; primaryLanguage: any; stargazerCount: any; url: any; }) => {
		return {
...
export const getMostStarredRepos = async (): Promise<IStarredRepo[]> => {
	...
	const repos: IStarredRepo[] = data.user.repositories.edges.map((r: { node: { description: any; name: any; primaryLanguage: any; stargazerCount: any; url: any; }; }) => {
		return {
...
export const getCategories = async (): Promise<ILabel[]> => {
	...
	const labels: ILabel[] = data.repository.labels.edges.map((l: { node: { color: any; name: any; id: any; }; }) => {
		return {
			color: l.node.color,
			name: l.node.name,
			id: l.node.id,
...
export const getPosts = async (): Promise<IPost[]> => {
	...
	const posts: IPost[] = data.repository.discussions.edges.map((e: { node: { labels: { edges: any[]; }; title: any; author: any; id: any; createdAt: moment.MomentInput | undefined; number: any; }; }) => {
...
export const getPinnedPosts = async (): Promise<IPost[]> => {
	...
	const posts: IPost[] = data.repository.pinnedDiscussions.edges.map((e: { node: { discussion: { labels: { edges: any[]; }; title: any; author: any; id: any; createdAt: moment.MomentInput | undefined; number: any; }; }; }) => {
...
export const getCommits = async (): Promise<ICommitsData> => {
	...
	data.repository.defaultBranchRef.target.history.edges.map((c: { node: { committedDate: moment.MomentInput | undefined; message: any; commitUrl: any; author: { name: any; }; }; }) => {
